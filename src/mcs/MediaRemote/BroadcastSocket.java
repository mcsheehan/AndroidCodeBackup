...
/*
 * This class tries to send a broadcast UDP packet over your wifi network to discover the MCSMedia remote service. 
 */

public class BroadcastSocket extends Thread {
	private static final String TAG = "Discovery";
	private static final String REMOTE_KEY = "b0xeeRem0tE!";
	private static final int DISCOVERY_PORT = 2565;
	...
	BroadcastSocket(WifiManager wifi) {
		mWifi = wifi;
	}
	...
	private void sendDiscoveryRequest(DatagramSocket socket) throws IOException {
		...
		InetAddress NetAddress = InetAddress.getByName("255.255.255.255");
		DatagramPacket packet = new DatagramPacket(data.getBytes(), data.length(),
				NetAddress, DISCOVERY_PORT);
		socket.send(packet);
	}
